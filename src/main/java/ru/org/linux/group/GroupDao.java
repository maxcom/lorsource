...
public class GroupDao {
  ...
  public Group getGroup(int id) throws BadGroupException {
    try {
      return jdbcTemplate.queryForObject(
        "SELECT sections.moderate, imagepost, vote, section, havelink, linktext, title, urlname, image, groups.restrict_topics, restrict_comments,stat1,stat3,groups.id, groups.info, groups.longinfo, groups.resolvable FROM groups, sections WHERE groups.id=? AND groups.section=sections.id",
        new RowMapper<Group>() {
          @Override
  ...
  public List<Group> getGroups(Section section) {
    return jdbcTemplate.query(
      "SELECT sections.moderate, imagepost, vote, section, havelink, linktext, title, urlname, image, groups.restrict_topics, restrict_comments, stat1,stat3,groups.id,groups.info,groups.longinfo,groups.resolvable FROM groups, sections WHERE sections.id=? AND groups.section=sections.id ORDER BY id",
      new RowMapper<Group>() {
        @Override
