...
<link rel="canonical" href="${template.mainUrlNoSlash}${l:getLinkPage(message, page)}">
...
<c:if test="${pages > 1}">
    ...
    <c:if test="${page != -1 and page != 0}">
        <c:set var="bufInfo" value="&emsp;<a class='page-number' href='${l:getLinkPage(message, page-1)}${filterAdd}'>←</a>" />
    </c:if>
    ...
    <c:if test="${page != -1 and page + 1 != pages}">
        <c:if test="${page + 1 == pages - 1}">
            <c:set var="bufInfo" value="${bufInfo} <a class='page-number' href='${l:getLinkPage(message, page+1)}${urlAdd}}'>→</a>" />
        </c:if>
        <c:if test="${page + 1 != pages - 1}">
            <c:set var="bufInfo" value="${bufInfo} <a class='page-number' href='${l:getLinkPage(message, page+1)}${filterAdd}}'>→</a>" />
        </c:if>
    </c:if>
...
<c:if test="${fn:length(commentsPrepared)!=unfilteredCount}">
  <div class=nav>
    Показано ${fn:length(commentsPrepared)} сообщений из ${unfilteredCount}. Показать <a href="${l:getLinkPage(message, page)}?filter=show">все</a>.
  </div>
</c:if>

<c:if test="${filterMode!=defaultFilterMode}">
  <div class=nav>
    Показаны все комментарии. <a href="${l:getLinkPage(message, page)}">Скрыть</a> игнорируемые.
  </div>
</c:if>
