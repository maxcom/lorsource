...
<b>Статус:</b> ${user.status}
 ...
 <c:if test="${user.blocked}"> (заблокирован)</c:if>
  ...
  <c:if test="${banInfo != null}">
    Блокирован <lor:date date="${banInfo.date}"/>, модератором <lor:user link="true" user="${banInfo.moderator}"/> по причине:
...
<c:if test="${not user.anonymous}">
  <b>Число комментариев: <c:if test="${not template.sessionAuthorized and userStat.commentCount>=1000}">приблизительно </c:if> ${userStat.commentCount}</b>
</c:if>
<p>
