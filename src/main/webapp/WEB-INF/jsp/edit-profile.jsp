...
<h2>Параметры профиля</h2>ddddddddd
...
<td><input type="checkbox" name="showSocial" <c:if test="${currentProperties.showSocial}">checked</c:if> ></td></tr>
<tr><td>Новые комментарии в начале</td>
<td><input type="checkbox" name="newfirst" <c:if test="${currentProperties.showNewFirst}">checked</c:if> ></td></tr>
<tr><td>Показывать фотографии</td>
<td><input type="checkbox" name="photos" <c:if test="${currentProperties.showPhotos}">checked</c:if> ></td></tr>
<tr><td><label for="topics">Число тем форума на странице</label> </td>
<td><input type="text" size="5" id="topics" name="topics" value="${currentProperties.topics}" ></td></tr>
<tr><td><label for="messages">Число комментариев на странице</label></td>
<td><input type="text" size="5" id="messages" name="messages" value="${currentProperties.messages}" ></td></tr>
<tr><td><label for="tags">Число меток в облаке</label></td>
<td><input type="text" size="5" id="tags" name="tags" value="${currentProperties.tags}" ></td></tr>
<%--
<tr><td>Верстка главной страницы в 3 колонки</td>
<td><input type=checkbox name=3column <c:if test="${currentProperties.threeColumnsOnMain}">checked</c:if> ></td></tr>
--%>
<tr><td>Показывать анонимные комментарии</td>
<td><input type="checkbox" name="showanonymous" <c:if test="${currentProperties.showAnonymous}">checked</c:if> ></td></tr>
<tr><td>Подсветка строчек в таблицах сообщений (tr:hover) (только для темы black)</td>
<td><input type="checkbox" name="hover" <c:if test="${currentProperties.useHover}">checked</c:if> ></td></tr>
<tr><td>Показывать меньше рекламы</td>
<td><input type="checkbox" name="hideAdsense" <c:if test="${currentProperties.hideAdsense}">checked</c:if> ></td></tr>
<tr><td>Показывать галерею в ленте на главной</td>
<td><input type="checkbox" name="mainGallery" <c:if test="${currentProperties.showGalleryOnMain}">checked</c:if> ></td></tr>
  <tr><td colspan=2><hr></td></tr>
<tr>
  <td valign=top>Тема</td>
  <td>
    <c:set value="${currentStyle}" var="style"/>
  ...
  <tr>
    ...
    <td>
      <c:set value="${currentProperties.avatarMode}" var="avatar"/>

      <c:forEach var="s" items="${avatarList}">
        <c:if test="${s == avatar}">
...
<tr>
  ...
  <td>
    <input type=radio name=format_mode id="format-quot"  value=quot <c:if test="${currentProperties.formatMode == 'quot'}">checked</c:if> ><label for="format-quot">TeX paragraphs (default)</label><br>
    <input type=radio name=format_mode id="format-ntobr" value=ntobr <c:if test="${currentProperties.formatMode == 'ntobr'}">checked</c:if> ><label for="format-ntobr">User line break</label><br>
  </td>
</tr>
