...
<tr>
  <td>
    ...
    <c:if test="${topic.pages>1}">
      (стр.
      <c:forEach var="i" begin="1" end="${topic.pages}"><c:out value=" "/><c:if test="${i==topic.pages and firstPage and year==null}"><a href="${group.url}${topic.msgid}/page${i}?lastmod=${topic.lastmod.time}">${i}</a></c:if><c:if test="${i!=topic.pages or not firstPage or year!=null}"><a href="${group.url}${topic.msgid}/page${i}">${i}</a></c:if></c:forEach>)
    </c:if>
