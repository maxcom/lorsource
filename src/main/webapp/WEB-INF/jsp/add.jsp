...
<c:if test="${not section.premoderated}">
  Метки (разделенные запятой, не более <%= TopicTagService.MAX_TAGS_PER_TOPIC %>; в заголовке будет показано не более <%= TopicTagService.MAX_TAGS_IN_TITLE %>, просим не злоупотреблять созданием новых тегов и использовать подходящие из наиболее популярных):<br>
</c:if>
