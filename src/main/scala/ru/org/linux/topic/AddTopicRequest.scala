...
class AddTopicRequest(
    ...
    @BeanProperty
    var poll: Array[String] = new Array[String](Poll.MaxPollSize),
    @BooleanBeanProperty
...
class EditTopicRequest(
    ...
    var tags: String = null,
    @BeanProperty
    var poll: util.Map[Integer, String] = null,
    @BeanProperty
    var editorBonus: util.Map[User, Integer] = null,
    @BeanProperty
    var newPoll: Array[String] = new Array[String](3),
    @BooleanBeanProperty
    var multiselect: Boolean = false,
    @BeanProperty
  ...
  def editorBonusScala: Map[User, Int] =
    Option(editorBonus)
      .map(_.asScala.view.mapValues(v => Option(v).map(_.toInt).getOrElse(0)).toMap)
      .getOrElse(Map.empty)
