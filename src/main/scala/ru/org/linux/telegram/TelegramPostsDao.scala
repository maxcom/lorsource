...
class TelegramPostsDao(ds:DataSource) {
  ...
  def hotTopic: Option[Topic] = {
    jdbcTemplate.queryAndMap(
      """
        |select topic, count (distinct comments.userid)
        |  from comments join users on comments.userid=users.id join topics on (comments.topic=topics.id)
        ...
        |group by topic having count (distinct comments.userid)>=15 order by count desc limit 1
