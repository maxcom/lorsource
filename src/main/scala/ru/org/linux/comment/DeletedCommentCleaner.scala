...
/** Периодическое окончательное удаление старых удалённых комментариев неактивных пользователей.
  ...
  * `del_info.deldate`; `deldate IS NULL` — удаления до начала ведения даты, сообщения до 2010 года — порог считается
  * заведомо пройденным), либо находится в топике, удалённом более 3 лет назад (или с `deldate IS NULL`), либо находится
  * в топике со скрытыми комментариями (`postscore = POSTSCORE_HIDE_COMMENTS`) с `lastmod` старше 3 лет, не имеет
  * ответов (включая удалённые), а его автор не заходил на сайт более 10 лет, либо заблокирован и не заходил более 3 лет,
  * либо не имеет дат регистрации и последнего входа (в т.ч. anonymous). Комментарии с удалёнными ответами вычищаются
  * постепенно: сначала листья цепочки, затем их родители.
  *
