...
class EditHistoryController(messageDao: TopicDao, editHistoryService: EditHistoryService,
  ...
  def showEditInfo(@PathVariable("id") msgid: Int): ModelAndView = AuthorizedOpt { currentUserOpt =>
    ...
    topicPermissionService.checkView(group, topic, currentUserOpt.map(_.user).orNull, preparedMessage.author, showDeleted = false)
    ...
    modelAndView.getModel.put("canRestore", groupPermissionService.isEditable(preparedMessage, currentUserOpt.map(_.user).orNull))
