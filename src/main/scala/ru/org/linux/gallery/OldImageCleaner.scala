...
/** Периодическое физическое удаление файлов старых удалённых изображений. Запускается раз в день.
  ...
  *   - '''Случай A''': все изображения топиков, удалённых более 3 лет назад (по `del_info.deldate`; `deldate IS NULL` —
  *     удаления до начала ведения даты, сообщения до 2010 года, порог считается заведомо пройденным). Файлы удаляются
