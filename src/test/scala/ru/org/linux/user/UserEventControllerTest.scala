...
class UserEventControllerTest extends FunSuite:
  ...
  test("validReactionRange"):
    ...
    assert(UserEventController.isValidClickRange(firstEvent, lastEvent))

  test("invalidReactionRangeForDifferentComment"):
    val firstEvent = event(id = 10, UserEventFilterEnum.REACTION, unread = true, topicId = 100, commentId = 200)
    val lastEvent = event(id = 12, UserEventFilterEnum.REACTION, unread = true, topicId = 100, commentId = 201)

    assert(!UserEventController.isValidClickRange(firstEvent, lastEvent))

  test("invalidSingleEventRange"):
