...
object Users:
  ...
  def getUser45ScoreBlocked(): ResultSet =
    val resultSet = mock(classOf[ResultSet])
  ...
  def getUserDefrosted(): ResultSet =
    val resultSet = mock(classOf[ResultSet])
    ...
    val past = new Timestamp(System.currentTimeMillis() - 1000)
  ...
  def getUserFrozen(): ResultSet =
    ...
    when(resultSet.getString("email")).thenReturn(null)
    when(resultSet.getInt("unread_events")).thenReturn(15)
    val future = new Timestamp(System.currentTimeMillis() + 1000)
    when(resultSet.getTimestamp("frozen_until")).thenReturn(future)
    when(resultSet.getInt("frozen_by")).thenReturn(1)
    when(resultSet.getString("freezing_reason")).thenReturn("just because")
    resultSet

  def getModeratorLowScore(): ResultSet =
    val resultSet = mock(classOf[ResultSet])
  ...
  def getAdministratorLowScore(): ResultSet =
    val resultSet = mock(classOf[ResultSet])
